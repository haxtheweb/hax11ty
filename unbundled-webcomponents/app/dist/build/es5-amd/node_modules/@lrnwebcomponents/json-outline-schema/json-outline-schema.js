 ...
 */var JsonOutlineSchema=/*#__PURE__*/function(_HTMLElement){babelHelpers.inherits(JsonOutlineSchema,_HTMLElement);var _super=_createSuper(JsonOutlineSchema);babelHelpers.createClass(JsonOutlineSchema,[{key:"html",// render function
get:function get(){return"\n<style>\n:host {\n  display: block;\n}\n:host([debug]) {\n  font-family: monospace;\n  white-space: pre;\n  margin: 16px 0px;\n}\n:host([hidden]) {\n  display: none;\n}\n        </style>\n<slot></slot>"}/**
   * Store the tag name to make it easier to obtain directly.
   * @notice function name must be here for tooling to operate correctly
   */}],[{key:"tag",get:function get(){return"json-outline-schema"}/**
   * life cycle
   */}]);function JsonOutlineSchema(){var _this,delayRender=0<arguments.length&&arguments[0]!==void 0?arguments[0]:!1;babelHelpers.classCallCheck(this,JsonOutlineSchema);_this=_super.call(this);// set tag for later use
_this.tag=JsonOutlineSchema.tag;_this.template=document.createElement("template");_this.attachShadow({mode:"open"});if(!delayRender){_this.render()}_this.__ready=!1;_this.file=null;_this.id=_this.generateUUID();_this.title="New site";_this.author="";_this.description="";_this.license="by-sa";_this.metadata={};_this.items=[];_this.debug=!1;window.JSONOutlineSchema.instance=babelHelpers.assertThisInitialized(_this);return _this}/**
   * life cycle, element is afixed to the DOM
   */babelHelpers.createClass(JsonOutlineSchema,[{key:"connectedCallback",value:function connectedCallback(){if(window.ShadyCSS){window.ShadyCSS.styleElement(this)}window.addEventListener("json-outline-schema-debug-toggle",this._toggleDebug.bind(this));var evt=new CustomEvent("json-outline-schema-ready",{bubbles:!0,cancelable:!1,composed:!0,detail:!0});this.dispatchEvent(evt);this.__ready=!0}},{key:"_copyAttribute",value:function _copyAttribute(name,to){var recipients=this.shadowRoot.querySelectorAll(to),value=this.getAttribute(name),fname=null==value?"removeAttribute":"setAttribute",_iterator=_createForOfIteratorHelper(recipients),_step;try{for(_iterator.s();!(_step=_iterator.n()).done;){var node=_step.value;node[fname](name,value)}}catch(err){_iterator.e(err)}finally{_iterator.f()}}},{key:"_setProperty",value:function _setProperty(_ref){var name=_ref.name,value=_ref.value;this[name]=value}},{key:"render",value:function render(){this.shadowRoot.innerHTML=null;this.template.innerHTML=this.html;if(window.ShadyCSS){window.ShadyCSS.prepareTemplate(this.template,this.tag)}this.shadowRoot.appendChild(this.template.content.cloneNode(!0))}/**
...
return!1}/**
   ...
   */},{key:"load",value:function(){var _load=babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee(location){var data,fileData,key,item,newItem;return regeneratorRuntime.wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:if(!location){_context.next=9;break}this.file=location;_context.next=4;return fetch(location).then(function(response){return response.text()});case 4:data=_context.sent;fileData=JSON.parse(data);for(key in fileData){if(babelHelpers.typeof(this[key])!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&"items"!==key){this[key]=fileData[key]}}// check for items and escalate to full JSONOutlineSchemaItem object
// also ensures data matches only what is supported
if(fileData.items){for(key in fileData.items){item=fileData.items[key];newItem=new _jsonOutlineSchemaItem.JSONOutlineSchemaItem;newItem.id=item.id;newItem.indent=item.indent;newItem.location=item.location;newItem.slug=item.slug;newItem.order=item.order;newItem.parent=item.parent;newItem.title=item.title;newItem.description=item.description;// metadata can be anything so whatever
newItem.metadata=item.metadata;this.items[key]=newItem}}return _context.abrupt("return",!0);case 9:return _context.abrupt("return",!1);case 10:case"end":return _context.stop();}}},_callee,this)}));function load(_x){return _load.apply(this,arguments)}return load}()/**
...
case"parent":props.component={name:"paper-input",valueProperty:"value",properties:{required:!0}};break;case"indent":case"order":props.component={name:"paper-input",valueProperty:"value",properties:{required:!0},attributes:{type:"number"}};case"metadata":case"items":props.type="array";props.items={type:"object",properties:{key:{title:"key",type:"string",component:{name:"paper-input",valueProperty:"value",properties:{required:!0}}},value:{title:"value",type:"string",component:{name:"paper-input",valueProperty:"value",properties:{required:!0}}}}};break;default:console.log(key);break;}schema.properties[key]=props}return schema}/**
   ...
   */},{key:"itemsToNodes",value:function itemsToNodes(){var items=0<arguments.length&&arguments[0]!==void 0?arguments[0]:[];if(0===items.length){items=this.items}var tree=this.unflattenItems(items);return this.treeToNodes(tree,document.createElement("ul"))}},{key:"treeToNodes",value:function treeToNodes(tree,appendTarget){for(var i in tree){var li=document.createElement("li");li.innerText=tree[i].title;li.setAttribute("data-jos-id",tree[i].id);if(tree[i].location){li.setAttribute("data-jos-location",tree[i].location)}if(tree[i].slug){li.setAttribute("data-jos-slug",tree[i].slug)}appendTarget.appendChild(li);if(tree[i].children&&0<tree[i].children.length){appendTarget.appendChild(this.treeToNodes(tree[i].children,document.createElement("ul")))}}return appendTarget}/**
   * Helper to unflatten an array and make it into a tree
   */},{key:"unflattenItems",value:function unflattenItems(items,parent,tree){var _this2=this;tree="undefined"!==typeof tree?tree:[];parent="undefined"!==typeof parent?parent:{id:null};var children=items.filter(function(child){return child.parent===parent.id});if(children.length){if(!parent.id){tree=children}else{parent.children=children}children.forEach(function(child){_this2.unflattenItems(items,child)})}return tree}/**
...
for(var i in node.children){node.removeAttribute("data-jos-id");node.removeAttribute("data-jos-location");node.removeAttribute("data-jos-slug");// deep scrub child references
...
if(child.getAttribute("data-jos-id")){item.id=child.getAttribute("data-jos-id")}if(child.getAttribute("data-jos-location")){item.location=child.getAttribute("data-jos-location")}else{item.location=""}if(child.getAttribute("data-jos-slug")){item.slug=child.getAttribute("data-jos-slug")}else{item.slug=""}item.indent=indent;item.order=order;order=order+1;// @todo mayyyyyy work but if nested structures may not for text
