define(["exports","../../../../mobx/lib/mobx.module.js","../../../utils/utils.js","../../../json-outline-schema/json-outline-schema.js"],function(_exports,_mobxModule,_utils,_jsonOutlineSchema){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.HAXCMSSiteStore=_exports.store=void 0;function _createSuper(Derived){var hasNativeReflectConstruct=_isNativeReflectConstruct();return function _createSuperInternal(){var Super=babelHelpers.getPrototypeOf(Derived),result;if(hasNativeReflectConstruct){var NewTarget=babelHelpers.getPrototypeOf(this).constructor;result=Reflect.construct(Super,arguments,NewTarget)}else{result=Super.apply(this,arguments)}return babelHelpers.possibleConstructorReturn(this,result)}}function _isNativeReflectConstruct(){if("undefined"===typeof Reflect||!Reflect.construct)return!1;if(Reflect.construct.sham)return!1;if("function"===typeof Proxy)return!0;try{Date.prototype.toString.call(Reflect.construct(Date,[],function(){}));return!0}catch(e){return!1}}var Store=/*#__PURE__*/function(){function Store(){babelHelpers.classCallCheck(this,Store);this.location=null;this.jwt=null;this.editMode=!1;this.manifest=null;this.activeItemContent="";this.themeElement=null;this.activeId=null;this.userData={};this.cmsSiteEditor={instance:null};this.cmsSiteEditorBackend={instance:null};this.dashboardOpened=!1}/**
   * Load a manifest / site.json / JSON outline schema
   * and prep it for usage in HAXcms
   */babelHelpers.createClass(Store,[{key:"loadManifest",value:function loadManifest(manifest){var target=1<arguments.length&&arguments[1]!==void 0?arguments[1]:null;// support a custom target or ensure event fires off window
if(null==target&&window){target=window}// @todo replace this with a schema version mapper
// once we have versions
if((0,_utils.varExists)(manifest,"metadata.siteName")){var git=(0,_utils.varGet)(manifest,"publishing.git",{});manifest.metadata.site={name:manifest.metadata.siteName,git:git,created:manifest.metadata.created,updated:manifest.metadata.updated};manifest.metadata.theme.variables={image:manifest.metadata.image,icon:manifest.metadata.icon,hexCode:manifest.metadata.hexCode,cssVariable:manifest.metadata.cssVariable};manifest.metadata.node={dynamicElementLoader:manifest.metadata.dynamicElementLoader,fields:manifest.metadata.fields};delete manifest.metadata.publishing;delete manifest.metadata.created;delete manifest.metadata.updated;delete manifest.metadata.siteName;delete manifest.metadata.image;delete manifest.metadata.icon;delete manifest.metadata.hexCode;delete manifest.metadata.cssVariable;delete manifest.metadata.dynamicElementLoader;delete manifest.metadata.fields}// repair slug not being in earlier builds of json schema
manifest.items.forEach(function(item,index,array){if(!item.slug){array[index].slug=item.location.replace("pages/","").replace("/index.html","")}});var site=new _jsonOutlineSchema.JsonOutlineSchema,nodes=site.itemsToNodes(manifest.items),correctOrder=site.nodesToItems(nodes),newItems=[];// we already have our items, pass them in
// build a new array in the correct order by pushing the old items around
for(var key in correctOrder){newItems.push(manifest.items.find(function(element){return element.id===correctOrder[key].id}))}manifest.items=newItems;this.manifest=manifest;target.dispatchEvent(new CustomEvent("json-outline-schema-changed",{bubbles:!0,composed:!0,cancelable:!1,detail:manifest}))}/**
   * Ensure there's a copy of the site-editor globally available
   */},{key:"cmsSiteEditorAvailability",value:function cmsSiteEditorAvailability(){if(!this.cmsSiteEditor.instance){this.cmsSiteEditor.instance=document.createElement("haxcms-site-editor")}return this.cmsSiteEditor.instance}},{key:"_computeItems",/**
...
if(this.manifest){var firstItem=this.manifest.items.find(function(i){return"undefined"!==typeof i.id});if(firstItem){return firstItem.slug}}return"/"}/**
...
var fields={title:this.activeItem.title,description:this.activeItem.description,location:this.activeItem.location,slug:this.activeItem.slug,created:this.activeItem.metadata.created,updated:this.activeItem.metadata.created};// mix in any custom field definitions
...
_exports.store=store;window.HAXCMS=window.HAXCMS||{};// request if this exists. This helps invoke the element existing in the dom
// as well as that there is only one of them. That way we can ensure everything
// is rendered through the same modal
window.HAXCMS.requestAvailability=function(){if(!window.HAXCMS.instance){window.HAXCMS.instance=document.createElement("haxcms-site-store");document.body.appendChild(window.HAXCMS.instance)}return window.HAXCMS.instance};// weird, but self appending
window.HAXCMS.requestAvailability();/**
 * HTMLElement
 */var HAXCMSSiteStore=/*#__PURE__*/function(_HTMLElement){babelHelpers.inherits(HAXCMSSiteStore,_HTMLElement);var _super=_createSuper(HAXCMSSiteStore);function HAXCMSSiteStore(){var _this7;babelHelpers.classCallCheck(this,HAXCMSSiteStore);_this7=_super.call(this);_this7.store=store;_this7.source="";/**
     * When location changes update activeItem
     */(0,_mobxModule.autorun)(function(){if(store.location&&store.location.route&&store.location.route.component){// get the id from the router
...
if(window.HaxStore.instance&&window.HaxStore.instance.globalPreferences.haxVoiceCommands){setTimeout(function(){window.HaxStore.instance.__hal.auto=!0},10)}}});return _this7}babelHelpers.createClass(HAXCMSSiteStore,[{key:"attributeChangedCallback",value:function attributeChangedCallback(name,oldVal,newVal){var _this8=this;if("source"==name&&""!=newVal){fetch(this[name]).then(function(response){return response.json()}).then(function(data){_this8.store.loadManifest(data)}).catch(function(err){console.warn(err)})}}},{key:"source",set:function set(value){this[name]=value;if(value){this.setAttribute("source",value)}},get:function get(){return this.getAttribute("source")}}],[{key:"tag",get:function get(){return"haxcms-site-store"}},{key:"observedAttributes",get:function get(){return["source"]}}]);return HAXCMSSiteStore}(/*#__PURE__*/babelHelpers.wrapNativeSuper(HTMLElement));_exports.HAXCMSSiteStore=HAXCMSSiteStore;window.customElements.define(HAXCMSSiteStore.tag,HAXCMSSiteStore)});