...
- running `npm run serve` to start a local HAXcms server

The `build/` directory contains the web component distribution (unbundled ES6 modules) and is kept in sync via the ubiquity build pipeline. The `wc-registry.json` maps tag names to their module paths for the magic-script autoloader. Custom themes live in `custom/` and are bundled via rollup.

### special note
The CDN mirrors for HAXcms may or may not include the elements you wish to extend your project with so those options for rapid distributed publishing may not be available.
